# backtester.py — ICT Kill Zone strategy backtester
...
class Backtester:
    ...
    def _pd_midpoint(self, levels: Optional[SessionLevels], kz_bars: pd.DataFrame) -> Optional[float]:
        # use London range for PD midpoint, fall back to KZ range
    ...
    @staticmethod
    def _simulate_trade(setup, forward_bars, rr, use_be=True, be_trigger_r=1.5):
        ...
        # Breakeven trigger price (+1.5R profit point)
        be_trigger_price = entry + risk * be_trigger_r if direction == 'LONG' else entry - risk * be_trigger_r
        current_sl = sl
        be_active = False
        ...
        for bar_time, bar in forward_bars.iterrows():
            ...
            if use_be and not be_active:
                if direction == 'LONG' and bar['high'] >= be_trigger_price:
                    current_sl = entry
                    be_active = True
                elif direction == 'SHORT' and bar['low'] <= be_trigger_price:
                    current_sl = entry
                    be_active = True
            ...
            if direction == 'LONG':
                if bar['low'] <= current_sl:
                    exit_price, exit_time = current_sl, bar_time
                    outcome = 'BE' if (be_active and current_sl == entry) else 'LOSS'
                    break
                if bar['high'] >= tp:
                    exit_price, exit_time = tp, bar_time
                    outcome = 'WIN'
                    break
            else:
        ...
        if outcome == 'OPEN':
            ...
            pnl_r_raw = (exit_price - entry) / risk if direction == 'LONG' else (entry - exit_price) / risk
            if pnl_r_raw >= rr * 0.99:
                outcome = 'WIN'
            elif pnl_r_raw <= -0.99:
                outcome = 'LOSS'
            elif abs(pnl_r_raw) < 0.1:
                outcome = 'BE'
            else:
                outcome = 'EOD_CLOSE'
    ...
    def calc_metrics(trades: List[dict]) -> dict:
        if not trades:
            return {
                'total': 0, 'wins': 0, 'losses': 0, 'be_count': 0, 'eod_count': 0,
        ...
        eod_trades = [t for t in trades if t['outcome'] == 'EOD_CLOSE']

        pnls = [t['pnl_r'] for t in trades]
        positive_pnls = [p for p in pnls if p > 0]
        negative_pnls = [p for p in pnls if p < 0]

        gross_profit = sum(positive_pnls) if positive_pnls else 0.0
        gross_loss = abs(sum(negative_pnls)) if negative_pnls else 0.0
        ...
        sharpe = float(pnl_arr.mean() / pnl_arr.std()) if len(pnl_arr) > 1 and pnl_arr.std() > 0 else 0.0

        # Win rate is strictly Full TP Wins / Total Trades
        strict_win_rate = round(len(wins) / len(trades) * 100, 1)
        ...
        return {
            ...
            'win_rate': strict_win_rate,
            'total_r': round(sum(pnls), 2),
            'avg_win_r': round(np.mean([t['pnl_r'] for t in wins]) if wins else 0, 2),
            'avg_loss_r': round(np.mean([t['pnl_r'] for t in losses]) if losses else 0, 2),
